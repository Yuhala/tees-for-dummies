...
As shown in the figure below, OP-TEE comprises two main components: `optee-os` which is the trusted side of the OP-TEE (executes in the secure world), and `optee_client`, which is the untrusted side of the OP-TEE (executes the normal world). The secure monitor bridges both components.
